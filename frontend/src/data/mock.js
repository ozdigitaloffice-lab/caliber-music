export const bandData = {
  name: "משפחת קליבר",
  nameEn: "CALIBER FAMILY",
  tagline: "היפ הופ ישראלי אמיתי",
  description: "משפחת קליבר - להקת הראפ שמייצגת את הסאונד האמיתי של הרחוב הישראלי. מוזיקה שנולדה מהלב, עם ביטים קשוחים ומילים שמדברות אל הנשמה.",
  heroImage: "https://customer-assets.emergentagent.com/job_3e445c6f-4f21-4280-b902-49885a77d5d7/artifacts/lt7sqz88_%D7%A2%D7%99%D7%A6%D7%95%D7%91%20%D7%9C%D7%9C%D7%90%20%D7%A9%D7%9D%20%2839%29.png",
  musicLinks: [
    {
      id: 1,
      platform: "Spotify",
      url: "https://open.spotify.com/artist/6yionLgW9s7T0cIrPWX7oB",
      color: "#1DB954",
      hoverColor: "#1ed760"
    },
    {
      id: 2,
      platform: "YouTube",
      url: "https://www.youtube.com/@%D7%9E%D7%A9%D7%A4%D7%97%D7%AA%D7%A7%D7%9C%D7%99%D7%91%D7%A8/shorts",
      color: "#FF0000",
      hoverColor: "#ff3333"
    },
    {
      id: 3,
      platform: "Apple Music",
      url: "https://music.apple.com/us/artist/%D7%9E%D7%A9%D7%A4%D7%97%D7%AA-%D7%A7%D7%9C%D7%99%D7%91%D7%A8/1873911260",
      color: "#FA243C",
      hoverColor: "#ff4d63"
    }
  ],
  stats: [
    { label: "שירים", value: "50+" },
    { label: "מאזינים", value: "100K+" },
    { label: "הופעות", value: "200+" }
  ],
  about: {
    title: "מי אנחנו",
    text: "משפחת קליבר היא לא סתם להקה - זו משפחה אמיתית שחיה ונושמת מוזיקה. מהרחובות של ישראל לבמות הגדולות, אנחנו מביאים את הסאונד הישראלי האותנטי. כל שיר, כל ביט, כל מילה - מגיע מהלב ומדבר את השפה של הרחוב.",
    quote: "\"המוזיקה שלנו היא הקול של הרחוב, הנשמה של המשפחה\""
  }
};
